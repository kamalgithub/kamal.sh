...
export interface CertificationCategory<S extends string = string> {
	slug: S;
	name: string;
}

export interface Certification<S extends string = string> extends Omit<Item<S>, 'shortDescription'> {
	color: string;
